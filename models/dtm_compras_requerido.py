    ...
    def get_view(self, view_id=None, view_type='form', **options):
        ...
        for material in get_info:
            ...
            if mapa2.get(material.codigo):
                ...
                listcant = [self.env['dtm.materials.line'].search(
                    [("model_id", "=", self.env['dtm.odt'].search([("ot_number", "=", odt),("tipe_order", "!=", 'PD')]).id),
                     ("materials_list", "=", material.codigo)],limit=1).materials_required for odt in listOdts]
            ...
            if len(material.orden_trabajo) > 3:
                lista = list(filter(lambda orden: self.env['dtm.materials.line'].search(
                    [("model_id", "=", self.env['dtm.odt'].search([("ot_number", "=", orden),("tipe_order", "!=", 'PD')],limit=1).id),
                     ("materials_list", "=", material.codigo)],limit=1).materials_required != 0,
        ...
        for orden in get_info:
            if len(orden.orden_trabajo) <= 3:
                get_odt = self.env['dtm.materials.line'].search([('model_id','=',self.env['dtm.odt'].search([('ot_number','=',orden.orden_trabajo),('tipe_order','=',orden.tipo_orden)],limit=1).id if self.env['dtm.odt'].search([('ot_number','=',orden.orden_trabajo)]) else 0),('materials_list','=',orden.codigo)])
